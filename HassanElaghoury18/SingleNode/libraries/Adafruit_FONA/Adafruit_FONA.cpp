...
//    if (! sendCheckReply(F("ATE0"), ok_reply) || !sendCheckReply(F("ATE0"), F("+CPIN: READY"))) {
//    return false;
//  }
...
uint8_t Adafruit_FONA::readline(uint16_t timeout, boolean multiline) {
  ...
  while (timeout--) {
    ...
    while(mySerial->available()) {
        //DEBUG_PRINTLN(F("mySerial is available"));
