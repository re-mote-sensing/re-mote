...
uint32_t feedDuration = 15000;						//time spent getting GPS fix
...
2) check sign of value and remove -ve sign if it exists
...
void WriteEE() {
	...
	DEBUG_PORT.println(nodeId);
	address = EEPROM.getAddress(sizeof(byte));
	EEPROM.writeByte(address, nodeId);
...
//	Wire.begin();
	...
	DEBUG_PORT.println();
	//writeConfig(loraPort, 0x0000, 0x0002);		// Edit the Nodes Network ID and Node ID
	readConfig(loraPort);
