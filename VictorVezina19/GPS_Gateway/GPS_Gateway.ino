...
#define MCU_LORA_RX 6 // Connects to LoRa TX, also is D7
#define MCU_LORA_TX 7 // Conencts to LoRa RX, also is D8
#define FONA_TX 4
#define FONA_RX 5
#define FONA_RST 9
...
void setup() {
	...
	//writeConfig(loraPort, 0x0000, 0x1234);		// Edit the Nodes Network ID and Node ID
	readConfig(loraPort);
...
//Sends HTTP request to server to post data
void postData() {
	fonaSS.listen();
	delay(100);
	...
	String request = "POST /ollie/sensor/info?id=7&name=GPS&coords=" + latS + "," + lngS + " HTTPS/1.1\r\nHost: www.cas.mcmaster.ca\r\n\r\n";
	char reqArr[request.length() + 1];
	request.toCharArray(reqArr, request.length() + 1);
	
    for (uint8_t i = 0; i < 3 && !fona.sendCheckReply(F("AT+CHTTPSSTART"), F("OK"), 30000); i++);
    delay(100);
    for(uint8_t i=0;i<3 && !fona.sendCheckReply(F("AT+CHTTPSOPSE=\"www.cas.mcmaster.ca\",80,1"), F("OK") ,10000);i++);
    delay(100);
    for (uint8_t i = 0; i < 3 && !fona.sendCheckReply(F("AT+CHTTPSSEND=120"), F(">"), 10000); i++);
    delay(100);
    for (uint8_t i = 0; i < 3 && !fona.sendCheckReply(reqArr, F("OK"), 10000); i++);
    delay(100);
    //for(uint8_t i=0;i<3 && !fona.sendCheckReply(F("AT+CHTTPSCLSE"),F("OK"),10000);i++);
    delay(100);
}
////////////////////////////////////
