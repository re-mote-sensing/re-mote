/*
 Name:		RFM95_MESH___Node.ino
 Created:	7/1/2019 2:16:13 PM
 Author:	tyrre
*/
...
RHMesh RHMeshManager(rf95, NODE_ID);								// Class to manage message delivery and receipt, using the driver declared above
...
uint8_t data[] = "Hello World!";			// Data to be tranmsitted
...
void loop() {
	...
	*/
	uint8_t error = RHMeshManager.sendtoWait(data, sizeof(data), GATEWAY_ID);
	Serial.println(error);

	/*
	Serial.println((ACK_MESH_RELAY_MESSAGE_TIMEOUT - (millis() - runtime)) > 0);
	if (ACK_MESH_RELAY_MESSAGE_TIMEOUT - (millis() - runtime > 10))
		delay(ACK_MESH_RELAY_MESSAGE_TIMEOUT - (millis() - runtime));
		*/

	Serial.println(F("START ACK"));
	...
	*/
	if (RHMeshManager.recvfromAckTimeout(buf, &len, ACK_MESH_NODE_MESSAGE_TIMEOUT, &from))
	{
